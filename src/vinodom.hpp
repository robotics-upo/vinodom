...
class VinOdom : public rclcpp::Node {
  ...
  VinOdom() : Node("vinodom") {
    ...
    RCLCPP_INFO(get_logger(), "Min plane dist: %f", minPlaneDist_);

    // Check topic name format
    if (camTopic_.back() == '/') camTopic_.pop_back();
    if (imuTopic_.back() == '/') imuTopic_.pop_back();
    if (altTopic_.back() == '/') altTopic_.pop_back();
    if (odomTopic_.back() == '/') odomTopic_.pop_back();
    ...
    imgSub_ = this->create_subscription<sensor_msgs::msg::Image>(
        camTopic_ + "/image_raw", 10, std::bind(&VinOdom::imageCallback, this, _1));
    cInfoSub_ = this->create_subscription<sensor_msgs::msg::CameraInfo>(
        camTopic_ + "/camera_info", rclcpp::SensorDataQoS(),
        std::bind(&VinOdom::cInfoCallback, this, _1));
    imuSub_ = this->create_subscription<sensor_msgs::msg::Imu>(
        imuTopic_, rclcpp::SensorDataQoS(), std::bind(&VinOdom::imuCallback, this, _1));
    altSub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
        altTopic_, rclcpp::SensorDataQoS(), std::bind(&VinOdom::altCallback, this, _1));
    barSub_ = this->create_subscription<sensor_msgs::msg::FluidPressure>(
        barTopic_, rclcpp::SensorDataQoS(), std::bind(&VinOdom::barCallback, this, _1));
  ...
  private:
  /**
   * @brief IMU callback. It performs innertial integration and stores IMU data to be used by
   * odometry
  ...
  void imuCallback(const sensor_msgs::msg::Imu::SharedPtr msg) {
    ...
    if (!tfImuCatched_) {
      try {
        ...
        RCLCPP_ERROR(this->get_logger(), "Could not transform %s to %s: %s", baseFrame_.c_str(),
...
public:
    ...
    ~VinOdom(void)
    {
>>>>>>> d47fed3d13b83cb3f8a48ca31fd84147cfa2901e
    }

    // Get orientation in base frame
    imuQ_ = imuBaseTf.getRotation() * tf2::Quaternion(msg->orientation.x, msg->orientation.y,
  ...
  void imageCallback(const sensor_msgs::msg::Image::SharedPtr msg) {
    ...
    if (!haveCalibration_ || !haveImu_ || !haveAlt_ || !haveBar_) return;
...
<<<<<<< HEAD
    ...
    if (height_ > barHeigh_) planeDist = barHeigh_;
...
>>>>>>> d47fed3d13b83cb3f8a48ca31fd84147cfa2901e
    ...
    double min = 100000, d;
    for (int i = 0; i < solutions; i++) {
      d = pow(normals_decomp[i].at<double>(0, 0), 2) + pow(normals_decomp[i].at<double>(1, 0), 2) +
          pow(normals_decomp[i].at<double>(2, 0) + 1, 2);
      if (d < min) {
        idx = i;
        min = d;
    ...
    if (showMatching_) {
      cv::Mat imgMatches;
      cv::drawMatches(cvbImg->image, kpts, kFrame_.img, kFrame_.kpts, matches, imgMatches,
  ...
  void openCVToTf(const cv::Mat &t, const cv::Mat &R, tf2::Transform &tf) {
    ...
    for (int i = 0; i < 3; ++i)
      for (int j = 0; j < 3; ++j) rotation_tf[i][j] = R.at<double>(i, j);
